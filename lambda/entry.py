...
def create_map(event):
  ...
  # Initialize the Cartopy Map used by each animation frame
  img_num = 0
  fig, ax = plt.subplots()
  ...
  for timestep, dataset in timesteps.items():
    ...
    for i in range(len(dataset['vals'])):
      digit = plt.text(dataset['lons'][i], dataset['lats'][i], dataset['vals'][i], fontsize=fontsize, ha='center', va='center')
      digits.append(digit)
